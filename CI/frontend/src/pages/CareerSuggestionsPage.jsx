          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1.5rem" }}>
            {suggestedSkills.map((skill, index) => (
              <motion.div key={skill} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}
                ...
                <div style={{ width: 50, height: 50, borderRadius: "15px", background: `${trackInfo.color}15`, color: trackInfo.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.8rem", flexShrink: 0 }}>
                  <TrendingUp size={26} />
                </div>
                <div>
                  <h4 style={{ margin: "0 0 0.5rem", fontSize: "1.2rem", fontWeight: 700, color: "#f1f5f9" }}>{skill}</h4>
                  <p style={{ margin: 0, fontSize: "0.95rem", color: "#64748b", lineHeight: 1.6 }}>Essential for progressing your career as a {displayTitle}.</p>
                </div>
              </motion.div>
            ))}
          </div>
